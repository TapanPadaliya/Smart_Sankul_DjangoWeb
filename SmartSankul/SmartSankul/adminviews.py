...
from app.models import Course,Session_Year,CustomUser,Student,Staff,Subject,Staff_Notification,Staff_leave,Staff_Feedback,Student_Notification,Student_Feedback,Student_leave,Attendance_Report,Attendance
...
def ADD_STUDENT(request):
    ...
    if request.method == "POST":
        ...
        if CustomUser.objects.filter(email=email).exists():
            messages.warning(request,'Email Is Alredy Exists')
            return redirect('add_student')
        if CustomUser.objects.filter(username=username).exists():
            messages.warning(request, 'Username Is Alredy Exists')
            return redirect('add_student')
            ...
            student.save()
            messages.success(request,user.first_name + " "+user.last_name +  " Are Saved Successfully")
            return redirect('add_student')
...
def ADD_COURSE(request):
    if request.method == "POST":
        ...
        course.save()
        messages.success(request,'Course Are Saved Successfully')
...
def UPDATE_STAFF(request):
    if request.method=="POST":
        ...
        staff.save()
        messages.success(request,'Stff is successfull updated')
...
def UPDATE_SUBJECT(request):
    if request.method=="POST":
        ...
        subject.save()
        messages.success(request,'Subject Are Successfully Update')
...
def DELETE_SUBJECT(request,id):
    ...
    subject.delete()
    messages.success(request,'Subject Are Deleted Successfully')
    return redirect('view_subject')
...
def SAVE_STUDENT_NOTIFICATION(request):
    ...
    if request.method == "POST":
        ...
        messages.success(request,'Notification Are Send Successfully',context)
...
def VIEW_ATTENDANCE(request):
    ...
    if action is not None and request.method == "POST":
        ...
        for i in attendace:
            attendace_id = i.id
            attendance_report = Attendance_Report.objects.filter(attendance_id=attendace_id)
    context = {
        'subject': subject,
        'session_year': session_year,
        'action': action,
